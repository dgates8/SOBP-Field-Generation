...
//tune hyperfine parameter to get higher pass rate, currently tuned to ~90% which is good enough for my application
#define eta .00031
...
//Finds the sum of each threadblock 
...
//computes the hadamard product for the dot product
...
//calculates each layer of error per neuron
...
//activates each neuron using a sigmoidal function
...
int main(){
	...
	//set device to run on, keep device 1 unoccupied
	cudaSetDevice(5);
	int GRIDSIZE = (mag+BLOCKSIZE-1)/(BLOCKSIZE);
	...
	//loop to learn from generated training data
	while(FLAG){
